...
- **Tmux 實時視效與狀態機**:
    ...
    - **輕量狀態檔**：即時百分比寫入 `~/.gemini/gcs-guardian/sessions/<session_id>/tmux_status`，供 `.tmux.conf` 內的 `status-right` 高頻讀取 (無效能開銷)。
...
| :--------------- | :----------------- | :--------- | :----------------------- |
| **SessionStart** | `gcs_init.sh`      | 啟動/重置時 | 注入 L4 骨架、將目前 tmux session 的狀態重置為 `[GCS: 0%]`。 |
